...
export const ServiceDialog = ({ children, buttonText }: { children: React.ReactNode, buttonText?: string }) => {
    ...
    return (
        <>
            <Button1
                onClick={open}
                className="!min-w-52 bg-color3 text-white py-2 rounded-md hover:bg-color3/60 transition duration-300"
            >
                {buttonText ?? "詳細を見る"}
            </Button1>
            ...
            <Dialog open={isOpen} as="div"
                ...
                <div className="fixed inset-0 w-screen">
                    <div className="flex min-h-full items-center justify-center p-4">
                        <DialogPanel
                            transition
                            className="overflow-auto flex flex-col z-50 max-h-[90vh] rounded-xl bg-white duration-300 ease-out data-[closed]:transform-[scale(95%)] data-[closed]:opacity-0"
                        >
                            {children}
                        </DialogPanel>
                    </div>
